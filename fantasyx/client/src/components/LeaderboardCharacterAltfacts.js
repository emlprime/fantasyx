...
class LeaderboardCharacterAltfacts extends Component {
    componentWillMount() {
        setTimeout(() => {this.getScores()}, 100);
    }

    getScores() {
        const msg = JSON.stringify({type: 'scores', options: {pivot: 'character', canon: 'altfacts'}})
...
const LeaderboardCharacterAltfactsContainer = connect(  
    mapStateToProps,
